(function(){
  ...
  var statusNode=null,worldStarted=false,worldAt=0,stable=0,shown=0,done=false,lastProbe=0,rawProgress=0,worldCanvas=null,lastRenderedPct=-1,lastRenderedScale=-1;
  ...
  function gpuWarm(){if(!worldStarted)return 0;if(!worldCanvas||!worldCanvas.isConnected){var cs=document.querySelectorAll('canvas');for(var i=0;i<cs.length;i++){if(cs[i]!==canvas&&cs[i].width>300&&cs[i].height>250){worldCanvas=cs[i];break}}}var painted=!!(worldCanvas&&worldCanvas.isConnected&&worldCanvas.width>0&&worldCanvas.height>0);stable=painted?Math.min(110,stable+1):0;return stable/110}
  function finish(){if(done)return;done=true;label.textContent='Island ready';pct.textContent='100%';fill.style.transform='scaleX(1)';setTimeout(function(){el.classList.add('jf-exit');setTimeout(function(){if(disposeFaller)disposeFaller();if(observer)observer.disconnect();if(el.parentNode)el.parentNode.removeChild(el);releaseAudio()},900)},220)}
  function frame(now){if(done)return;var dt=Math.min(.034,(now-last)/1000);last=now;if(!use3d){resize();ctx.clearRect(0,0,w,h);drawCharacter(dt,now)}if(now-lastProbe>120){lastProbe=now;hideOriginal();holdMedia();syncAudioState();rawProgress=sourceProgress();if(now-start>2200&&(rawProgress>=99||travelButton())&&!worldStarted)startWorld()}var value=worldStarted?82+18*gpuWarm():Math.min(82,rawProgress*.82);shown=Math.max(shown,Math.min(99.5,value));var whole=Math.floor(shown),scale=shown/100;if(whole!==lastRenderedPct){lastRenderedPct=whole;pct.textContent=whole+'%'}if(Math.abs(scale-lastRenderedScale)>.002){lastRenderedScale=scale;fill.style.transform='scaleX('+scale+')'}if(modelReady&&worldStarted&&stable>=110&&now-worldAt>1500&&now-start>(reduced?1000:4400))finish();else requestAnimationFrame(frame)}
