...
export class LandingArea extends Area
{
    ...
    setByteJayLetters()
    {
        ...
        // Keep the original world composition and physics untouched everywhere else,
        // but swap the landing signature itself for ByteJay.
        for(const reference of references)
        {
        ...
        const start = references[0].position
        const end = references[references.length - 1].position
        const center = new THREE.Vector3()
        for(const reference of references)
            center.add(reference.position)
        center.multiplyScalar(1 / references.length)

        const xAxis = start.clone().sub(end)
        xAxis.y = 0
        xAxis.normalize()
        ...
        new FontLoader().load('fonts/helvetiker_bold.typeface.json', (font) =>
        {
            ...
            const colors = [ '#ff735f', '#ffb24a', '#ffdf6c', '#84d68e', '#69b8ff', '#aa7cff', '#ff85be' ]
            const characters = 'BYTEJAY'
            const meshes = []
            let cursor = 0

            for(let i = 0; i < characters.length; i++)
            {
                const geometry = new TextGeometry(characters[i], {
                    font,
                    size: 2.15,
                    depth: 0.46,
                    curveSegments: 2,
            ...
            group.position.addScaledVector(xAxis, - cursor * 0.5)
            group.visible = false
            ...
            {
                if(this.game.world.intro === null)
                {
                    group.visible = true
                    this.game.ticker.events.off('tick', revealWhenReady)
                }
            }
            this.game.ticker.events.on('tick', revealWhenReady, 10)
        })
    }
